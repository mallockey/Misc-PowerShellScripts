...
write-host "Working, please wait..."
	foreach ($computers in $computersArray){
		...
		get-hotfix -computerName $computers | select  pscomputername, hotfixid, installedon | sort installedon  | export-csv $scriptLoc\WindowsUpdates.csv -append -notypeinformation
		}
